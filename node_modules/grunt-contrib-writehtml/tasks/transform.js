...
function html ( configuration ) {
  ...
  try {
    ...
    var configFile = grunt.option( 'source' ) + '/json/conf.json';
    ...
    if ( grunt.file.isFile( grunt.option( 'source' ) + '/json/widgets.json' ) ) {
      widgets = grunt.file.readJSON ( grunt.option( 'source' ) + '/json/widgets.json') ;
    }
    ...
    if ( grunt.file.isDir ( grunt.option( 'source' ) + '/json/pages' ) ) {
      var sources = fs.readdirSync ( grunt.option( 'source' ) + '/json/pages' ) ;
      for ( var i = 0; i < sources.length ; i++ ) {
        if ( sources[i].match('.json') ) {
          _.extend ( pages, grunt.file.readJSON ( grunt.option( 'source' ) + '/json/pages/' + sources[i] ) ) ;
        }
      }
    }
    ...
    if ( matchWidgets && matchWidgets[0] ) {
      ...
      _.each ( toJSON.hbs, function ( hbs ) {
        ...
        if ( grunt.file.isFile(grunt.option( 'source' ) + '/views/' + hbs.template ) ) {
        
          var rawTemplate = grunt.file.read( grunt.option( 'source' ) + '/views/' + hbs.template ) ;
      ...
      _.each ( toJSON.js, function ( js ) {
    	...
    	if ( jsConfiguration.js.style == 'compressed' ) {
          ...
          if ( grunt.file.isFile ( grunt.option( 'destination' ) + '/js/' + js_filename ) ) {
            source.closure += '<script defer>' + grunt.file.read( grunt.option( 'destination' ) + '/js/' + js_filename ) + '</script>' ;
          }
        }
    	else {	  
          if ( grunt.file.isFile ( grunt.option( 'destination' ) + '/js/' + js ) ) {
    ...
    if ( sassConfiguration.dist.build === 'external' ) {
      ...
      source.css = "<style>" + grunt.file.read (grunt.option( 'destination' ) + '/css/style.css') + "</style>";
    }
    ...
    grunt.file.recurse ( grunt.option( 'source' ) + '/views/', function callback (abspath, rootdir, subdir, filename ) {
          ...
          if ( matchWidgets && matchWidgets[0] ) {
            ...
            _.each ( toJSON.js, function ( js ) {
              if ( jsConfiguration.js.style == 'compressed' ) {
            	var js_filename = path.basename ( js , path.extname( js ) ) + '.min' + path.extname ( js ) ;
                if ( grunt.file.isFile ( grunt.option( 'destination' ) + '/js/' + js_filename ) ) {
                  javascriptString += '<script>' + grunt.file.read( grunt.option( 'destination' ) + '/js/' + js_filename ) + '</script>' ;
                }
              }
              else {
                if ( grunt.file.isFile( grunt.option( 'destination' ) + '/js/' + js ) ) {
    ...
    grunt.file.recurse ( grunt.option( 'source' ) + '/views/', function callback(abspath, rootdir, subdir, filename ) {
      if ( filename.match('.hbs') ) {
        grunt.file.write ( grunt.option( 'destination' ) + '/js/' + filename, grunt.file.read( abspath ) ) ;
      }
    });
    ...
    var rewriteBase = hogan.compile ( grunt.file.read ( grunt.option( 'source' ) + '/views/htaccess.mustache' ) ) ;
    
    var rewriteBasePath = source.appRoot + configuration.route.replace('/index.html' , '' ) ;

    var htaccessFilename = grunt.option( 'destination' ) + configuration.route.replace('/index.html' , '') + '/.htaccess' ;

    /** leave data behind */    
    grunt.file.write( grunt.option( 'source' ) + '/json/datasources/' + traceName + '.json', JSON.stringify ( source ) ) ;
    ...
    /** write HTML file */
    grunt.file.write( grunt.option( 'destination' ) + '' + configuration.route, htmlminify ( handlebars_template ( source ), htmlminifyConfiguration ) ) ;
