...
module.exports = function ( grunt ) {
  ...
  function writehtml () {
    ...
    try {
      ...
      var pagesDir = grunt.option( 'source' ) + '/json/pages';
      ...
      grunt.file.recurse(grunt.option( 'source' ) + '/js/', function callback ( abspath, rootdir, subdir, filename ) {
        if ( filename.match('.js') ) {
          grunt.file.copy( abspath, grunt.option( 'destination' ) + '/js/' + filename );
        }
      });
      ...
      _.each ( pages , function ( element, index ) {
        ...
        if ( ! _.isUndefined ( pages[index].callback )  ) {
          ...
          transform.html ( { route : element.route , template : grunt.option( 'source' ) + '/views/' + index + '.mustache' , data : element } ) ;
        }
      } ) ;
    }
    catch ( err ) {
