...
function html ( configuration ) {
  ...
  try {
    ...
    var source = grunt.file.readJSON ( configFile ) ;
    
    var widgets = { } ;
    
    if ( grunt.file.isFile( grunt.option( 'sourceDir' ) + '/json/widgets.json' ) ) {
      widgets = grunt.file.readJSON ( grunt.option( 'sourceDir' ) + '/json/widgets.json') ;
    }
    ...
    if ( grunt.file.isDir ( grunt.option( 'sourceDir' ) + '/json/pages' ) ) {
      var sources = fs.readdirSync ( grunt.option( 'sourceDir' ) + '/json/pages' ) ;
      for ( var i = 0; i < sources.length ; i++ ) {
        if ( sources[i].match('.json') ) {
          _.extend ( pages, grunt.file.readJSON ( grunt.option( 'sourceDir' ) + '/json/pages/' + sources[i] ) ) ;
        }
      }
    }
      ...
      _.each ( toJSON.js, function ( js ) {
    	...
    	if ( jsConfiguration.js.style == 'compressed' ) {
          ...
          if ( grunt.file.isFile ( grunt.option( 'destinationDir' ) + '/js/' + js_filename ) ) {
            source.closure += '<script defer>' + grunt.file.read( grunt.option( 'destinationDir' ) + '/js/' + js_filename ) + '</script>' ;
          }
        }
    	else {	  
    ...
    grunt.file.recurse ( grunt.option( 'sourceDir' ) + '/views/', function callback (abspath, rootdir, subdir, filename ) {
    ...
    var rewriteBase = hogan.compile ( grunt.file.read ( grunt.option( 'sourceDir' ) + '/views/htaccess.mustache' ) ) ;
    ...
    /** leave data behind */    
    grunt.file.write( grunt.option( 'sourceDir' ) + '/json/datasources/' + traceName + '.json', JSON.stringify ( source ) ) ;
